...
def print_text():
    while True:
        ...
        if str(datetime.datetime.now())[17:19] == '00':
            ...
            protectTrip = read_file('trip_protect')
            signList = ['0'] * len(signList)
            write_file('sign', signList)
            for s in protectTrip:
                num = nameList.index(s)
                moneyList[num] = str(eval(moneyList[num]) - 0.01)
            write_file('money', moneyList)
        if str(datetime.datetime.now())[14:19] == '00:00':
            nameList = read_file('name')
            moneyList = read_file('money')
            hour = int(str(datetime.datetime.now())[11:13])
            xc.send_message('''定点报时：现在是北京时间{hour}点整！
---
欢迎前往我主人的论坛：https://forum.sqj.repl.co/
也欢迎来我主人的mc服务器：https://mc.sqj.repl.co/
---
此外，我还要随机抽取一位幸运观众奖励==1000==个BB币，~~然后踢出。~~
'''.format(hour=hour))
            num = random.randint(0, len(nameList) - 1)
            kick = nameList[num]
            moneyList[num] = str(eval(moneyList[num]) + 1000)
            write_file('money', moneyList)
            xc.send_message('这位幸运观众就是{name}!\n@{name} ==1000==个BB币已经汇到你的账户上了'.format(name=kick))
...
def message_got(message, sender, trip, online, w=False):
    ...
    signList = read_file('sign')
    tripList = read_file('trip')
    tripProtect = read_file('trip_protect')
    while len(tripList) < len(nameList):
        tripList.append('NaN')
    write_file('trip', tripList)
    ...
    if tripList[num] != trip:
        if sender not in tripProtect:
            tripList[num] = trip
            write_file('trip', tripList)
        else:
            xc.send_message('@{name} 你的识别码已绑定[{trip}]，所以你可能是冒牌货，一路走好！'.format(name=sender, trip=tripList[num]))
            xc.send_to('ZhangHelper', '^kick <name>'.format(name=sender))
    for a in range(len(moneyList) - 1):
        ...
        for i in range(len(moneyList) - 1 - a):
            if eval(moneyList[i]) < eval(moneyList[i + 1]):
                ...
                tripList[i], tripList[i + 1] = tripList[i + 1], tripList[i]
                flag = 1
        if flag == 0:
            break
    write_file('name', nameList)
    ...
    if message[0:4] == '.bb ':
        if wList[1] == 'help':
            if trip == 'F1n+S1':
                yId = 'Admin'
            elif trip in modTrip:
                yId = 'Mod'
            elif trip in trustTrip:
                yId = 'Trust'
            else:
                yId = 'User'
            xc.send_to(sender, '你的身份: {id}'.format(id=yId))
            xc.send_to(sender, """.
下列指令可以使用：
-----User-----
1、.bb help：获取帮助
2、.bb shoot <name>: 花==50==个BB币向<name>射击
3、.bb hug <name>: 抱抱<name>
4、.bb cookie <name>: 花==25==个BB币买曲奇饼干送给<name>
5、.bb roll: 生成1～1000的随机数
6、.bb join: 加入BBI警卫队
7、.bb quit: 退出BBI警卫队
8、.bb bbi: 查看BBI警卫队的成员
9、/w BBot shoot <name>: 花==10==个BB币向<name>射箭
10、.bb readme: 聊天室使用守则
11、.bb online: 查看在线用户
12、.bb rkick <name>: 花费==500==个BB币与<name>决斗（有65%的可能自己被踢，30%的可能<name>被踢，5%的可能在线用户中的随机一人被踢）
13、.bb money: 查看自己的资产
14、.bb sign: 每分钟签到领取==10*level==个BB币
15、.bb send <name> <money>: 送给<name><money>个BB币
16、.bb most：查看福布斯富豪榜
17、.bb level: 查看自己的等级
18、.bb upgrade: 花费==$2^{level}$==个BB币升级
19、.bb readbot：查看BBot使用守则
20、.bb buy trust: 花费==50000==个BB币购买信任用户权限
21、.bb protectme: 申请保护盗号（首付==100==个BB币，后每分钟==0.01==个BB币）
22、.bb giveup: 取消保护盗号
23、.bb look: 查看申请保护盗号的用户
----Trust ----
...
5、.bb check <name>: 查看<name>的资产""")
        if wList[1] == 'protectme':
            if eval(moneyList[num]) >= 100:
                moneyList[num] = str(eval(moneyList[num]) - 100)
                write_file('money', moneyList)
                if sender not in tripProtect:
                    tripProtect.append(sender)
                    write_file('trip_protect', tripProtect)
                    xc.send_message('@{name} 申请成功！'.format(name=sender))
                else:
                    xc.send_message('@{name} 你早已经被保护了！不过放心，我不会退钱的。'.format(name=sender))
            else:
                xc.send_message('余额不足！')
        if wList[1] == 'giveup':
            if sender in tripProtect:
                tripProtect.remove(sender)
                write_file('trip_protect', tripProtect)
                xc.send_message('@{name} 取消成功！'.format(name=sender))
            else:
                xc.send_message('@{name} 你又没有被保护，取消个屁啊！'.format(name=sender))
        if wList[1] == 'look':
            send = '受保护的用户有：`'
            for s in tripProtect:
                send += s + '`，`'
            xc.send_message(send[:-2])
        if wList[1] == 'readbot':
            xc.send_to(sender, '''.
# BBot帮助文件（版本：1.00）
###### 更新日期：北京时间 2022-10-3 傍晚
## 前言  
BBot，是一个基于XChat聊天平台的机器人，由Blaze编写。它有着艰辛的发展历程。为推动XChat发展，Blaze特意编写了本帮助文件，希望诸位用户能细细阅读。
###### 小提示：某些人只会胡乱贬低BBot，请不要相信他们。
## 基本信息
BBot的识别码一般为BBking，昵称为BBot。
## BB币
### 获取BB币的方法
1. 签到（`.bb sign`）：每次签到可以获得10*level个BB币。
2. 加入BBI警卫队（`.bb join`）：加入立即获得100个BB币。
### 赠送BB币的方法：
见`.bb help`。
## 等级
### 如何升级
发送`.bb upgrade`，每次升级所需的BB币为2的现有等级次方。
###### 小提示：一开始等级为0，这是无论如何签到所获BB币的数量均为==0==，所以需要先加入BBI警卫队获取初始资金进行升级！！！
## 保护自己免受盗号
发送`.bb protectme`，先得付款100个BB币，后每分钟收取0.01个BB币的保护费。
如想取消，发送`.bb giveup`。
## 高风险行为
### 禁止
1. 开多个账号同时加入BBI
2. 轰炸BBot
3. 使用机器人在`?xq102210`公屏上发送大量签到（建议私信或前往`?bot`）

### 不建议（在某些情况下，可能会被管理员当作违规处理）
1. 同时加入多个签到机器人
2. 过度使用BBot（包括BBot的一切功能）

###### 小提示：人少就是违规的的理由！！！
==如果违反以上规定，可能会受到50BB币至100000BB币的罚款！==

## 结束语
我希望此帮助对您有帮助，如有疑问，请联系XChat用户Blaze。
感谢您的阅读，祝您聊天愉快！
---
感谢有你，陪伴着 BBot 走过风风雨雨；感谢有你，激励着 BBot 不断前进；感谢有你，信任 BBot 。
发展路上，感谢有你。''')
        if wList[1] == 'level':
            level = levelList[num]
            if w:
                ...
                xc.send_message('你的等级为{level}级'.format(level=level))
        if wList[1] == 'upgrade':
            ...
            if eval(moneyList[num]) >= 2 ** level:
                ...
                if not w:
                    ...
                    xc.send_to(sender,
                               '@{name} 你的等级已经升级到了{level}级'.format(name=sender, level=levelList[num]))
            else:
                if not w:
                    xc.send_message('余额不足')
                else:
        ...
        if wList[1] == 'money':
            ...
            if w:
                ...
                xc.send_message('您的现有资产为{money}个BB币。'.format(money=money))
        if wList[1] == 'readme':
            xc.send_to(sender, '''.
# XChat帮助文件（版本：2.21）
...
发展路上，感谢有你。''')
        ...
        if wList[1] == 'shoot' and '&#8238;' not in message:
            if eval(moneyList[num]) >= 50:
                shoot = message[10:]
                moneyList[num] = str(eval(moneyList[num]) - 50)
                write_file('money', moneyList)
        ...
        if wList[1] == 'cookie':
            if eval(moneyList[num]) >= 25:
                cookie = message[11:]
                moneyList[num] = str(eval(moneyList[num]) - 25)
                write_file('money', moneyList)
                xc.send_message('{user}花25块给{cookie}买了一个曲奇饼干'.format(user=sender, cookie=cookie))
            else:
                xc.send_message('余额不足')
        if wList[1] == 'roll':
        ...
        if wList[1] == 'rkick':
            if w:
                xc.send_to(sender, 'emmm，私信踢人是非常卑鄙的，有本事你就直接发出来啊！')
                return 0
            if eval(moneyList[num]) >= 500:
                moneyList[num] = str(eval(moneyList[num]) - 500)
                write_file('money', moneyList)
                name = message[10:]
        ...
        if wList[1] == 'buy':
            buyThing = wList[2]
            if buyThing == 'trust':
                if eval(moneyList[num]) >= 50000:
                    moneyList[num] = str(eval(moneyList[num]) - 50000)
                    write_file('money', moneyList)
                    if trip not in trustTrip:
                        trustTrip.append(trip)
                        write_file('trust', trustTrip)
                        xc.send_message("{name}花费50000个BB币将自己升级到了信任用户。".format(name=sender))
                    else:
                        xc.send_message("@{name} 你已经是本机器人的信任用户了。\n但是放心，既然你买了，我不会退钱的。".format(name=sender))
                else:
                    xc.send_message('余额不足')
        ...
        if trip == 'F1n+S1':
            ...
            if wList[1] == 'give':
                ...
                if name not in nameList:
                    ...
                    xc.send_message(
                        '@{name} 你收到了我主人发来的{money}个BB币。==输入`.bb readbot`查看如何使用BB币=='.format(name=name,
                                                                                                           money=money))
                    write_file('money', moneyList)
            if wList[1] == 'check':
                name = wList[2]
                if name in nameList:
                    money = moneyList[nameList.index(name)]
                    xc.send_message('{name}的资产为{money}个BB币'.format(name=name, money=money))
                else:
                    xc.send_message('未找到此用户！')
...
def whisper_got(message, nick, trip):
    ...
    if wList[0] == 'shoot' and '&#8238;' not in message:
        shoot = message[6:]
        if nick in nameList:
            if int(moneyList[nameList.index(nick)]) >= 10:
                moneyList[nameList.index(nick)] = str(eval(moneyList[nameList.index(nick)]) - 10)
                write_file('money', moneyList)
...
# xc = XChat.XChat("BlazeRobot", "xq102210", "BBot", '080314')  # 实例化类，要提供4个参数，分别是：机器人的token（请向XChat管理员申请）、聊天室名称、客户端昵称、可选密码。
xc = XChat.XChat("BlazeRobot", "xq102210", "BBot", '080314')
xc.message_function += [
